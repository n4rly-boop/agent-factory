...
These are the INTEGRATION seams between af.lifecycle and af.squad, not the squad store in
isolation (that is test_squad.py) nor revive's refusal matrix (that is test_revive.py):
  ...
  * revive() reads squad.get(name).live_sid FIRST in its sid-resolution chain, ahead of the
             once-written sid file and the spec's frozen spawn-time id.
  * up()     records the station in the roster, but ADDITIVELY: a squad-write failure must
...
from af import lifecycle, spec as af_spec, squad
...
class SquadWiring(TempFactory):
    ...
    def test_down_captures_live_sid_before_kill(self):
        ...
        squad.mark_up("w", self.p, live_sid=SPAWN_SID, role="worker")
    ...
    def test_revive_prefers_squad_live_sid_over_the_sid_file(self):
        ...
        self._spec()
        squad.mark_up("w", self.p, live_sid=FORK_SID, role="worker")
    ...
    def test_revive_falls_through_to_the_sid_file_when_no_squad_station(self):
        # Back-compat: an agent spawned before the roster existed has no squad.json entry. The
        # squad lookup must return nothing and NOT crash — revive falls through to the sid file.
        self._spec()
        self.assertIsNone(squad.get("w", self.p))         # nothing on the roster
    ...
    def test_up_survives_a_squad_write_failure(self):
        # The roster is an additive convenience view during the migration; the sid file stays
        # authoritative. So a squad.mark_up that throws must be swallowed — the launch itself
        ...
        with mock.patch.object(af_paths, "SPEC_HOME", self.root / "spechome"), \
                ...
                mock.patch("af.lifecycle.squad.mark_up",
